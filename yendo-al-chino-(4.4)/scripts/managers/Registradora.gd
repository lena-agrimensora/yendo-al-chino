...
#TODO: no esperar que el list manager este siempre aca :3
...
func _ready() -> void:
	...
	for mandado in mandados:
		mandados_instances.append(mandado)
...
func _on_body_entered(body: Node3D) -> void:
	if body.name == "Carrito":
		...
		var mandados_nombres = mandados
		var inventario_nombres: Array = []
		for p in canasto_inventario:
			inventario_nombres.append(str(p.name))
...
func _cambiar_a_victoria():
	#TODO: no hardcodear maaas!!
	get_tree().change_scene_to_file("res://scenes/interface/Victoria.tscn")
